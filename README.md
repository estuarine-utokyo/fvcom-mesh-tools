...
* **rung 3** — rung 2 + H: per-element greedy quality optimiser
  (`phase_h_optimize`). Off by default — opt in via
  `--max-iters 4 --phase-h ...` (optionally
  `--phase-h-coastline /path/to/coast.shp` for v3-style
  coastline projection on new boundary nodes). Expensive
  (10-30 min on a 47 k-element mesh) but consistently lifts
  alpha p05 ~+0.03 and drives `frac<20°` to 0 % on the
  Tokyo Bay benchmark (PoCs #41 / #42 / #43).
...
| 44 | `44_phase_h_2step_dry_run_poc.py` | 2-step lookahead **dry-run** (read-only) on the v3 residual (`outputs/43_phase_h_v3_optimized.14`, 11,015 fail elements). For each of 1,000 random-sampled fail elements (seed=42), the PoC tries (a) 1-step strict gate on E (must fail — sanity ✓ for v3) and (b) every `(op1, op2)` pair with op1 ∈ Phase H v3 inventory applied with `force=True` (penalty gate bypassed; validity preserved) and op2 ∈ `{smooth_node, edge_swap}` accepted iff the **union-penalty** over (op1.affected ∪ op2.affected) nodes strictly drops vs the initial mesh. Result (4,237 s wall on 1 core): 1-step 0 (0.0 %) / **2-step 610 (61.0 %)** / unfixable 390 (39.0 %). 95 % CI on the 61 % rate is ±3.0 %. Only **two distinct accepted pairs** appear: `vertex_remove + smooth_node` (326, 53 %) and `smooth_node + smooth_node` (284, 47 %); `edge_swap`/`edge_split_*` never accepted as either step. The structure is uniform — a "barrier-crossing" op1 (displace or remove one interior vertex even though local penalty rises) followed by a `smooth_node` recoupment over the perturbed ring. Extrapolated: 11,015 × 0.61 ≈ 6,700 additional fixable elements, residual ≈ 4,300. GO signal for Phase H v4 driver with the restricted op1 ∈ `{smooth_node, vertex_remove}` × op2 = `smooth_node` inventory; **however PoC #45 below disproves this extrapolation under the iterative driver.** Also added `force=False` kwarg to all five v3 operators with regression tests. |
| 45 | `45_phase_h_v4_validation_poc.py` | End-to-end validation of Phase H v4 (`phase_h_optimize(..., lookahead_enabled=True, max_lookahead_per_round=2000)`) on the same pipeline-rung-1 input + Tokyo-Bay coastline used by PoC #43. **Negative result.** Pass C accepted 20,000 lookahead pairs across 10 outer rounds (cap 2 000 each; 19,831 `smooth_node + smooth_node`, 169 `vertex_remove + smooth_node`) yet the headline quality regressed marginally and the fail count grew (v3 11,015 → v4 11,080, +65) at **12.1× the wall time** (1 735 s → 20 914 s). The PoC #44 dry-run's extrapolated +61 % fixable did **not** materialise in the iterative driver. Three concurring root causes: (i) the union-penalty gate sums over only the op1 ∪ op2 1-ring elements (~6–12 elements) and misses 2-ring drift, (ii) the gate accepts on *any* strict drop including O(10⁻³) "improvements" that do not exit the target element from fail status, and (iii) Pass A re-smoothing in the next outer round partially undoes Pass C's perturbations — the system thrashes (Pass C 2 000 accepts ↔ Pass A 213 sweeps × 10 rounds, net ≈ 0). v4 is therefore **not recommended**; `--phase-h-lookahead` stays off by default and is kept as a known-bad baseline for the next iteration. Viable v4.1 fixes: (a) **strictly-fixes-target gate** — accept iff E exits fail status on m2 (the SMS manual-edit standard), or (b) **2-ring union** — extend the gate's elements to the 2-ring of (op1 ∪ op2 affected nodes). The original PoC #44 caveat — that cluster-scale *patch re-CDT* is the more promising direction — is reinforced by this negative result. |
...
| `fmesh-mesh-pipeline in.14 out.14 [--bbox] [--max-iters N] [--best-rung] [--phase-h ...] [--min-alpha F] [--max-frac-lt-20deg F] [--max-valence N] [--max-flipped N] ...` | Progressive `clean → quality → repeat` loop. Applies up to four cumulative rungs of `fmesh-mesh-clean` phases — rung 0 (A+B+C), rung 1 (+D+F+G), rung 2 (+E), rung 3 (+H) — evaluating `fmesh-mesh-quality` thresholds after each. Default `--max-iters 3` stops at rung 2; opt into rung 3 (Phase H per-element greedy optimiser) via `--max-iters 4 --phase-h` plus optional `--phase-h-coastline /path/to/coast.shp` for v3-style boundary projection. Stops at the first passing rung by default; with `--best-rung`, runs every rung up to `--max-iters` and outputs the gate-passing rung with the highest `alpha_mean` (ties broken in favour of the lighter repair). Exits 1 if no rung satisfies the gate when thresholds are supplied. JSON history records per-rung metrics and threshold-check results. |
