...
| 34 | `34_wavelength_sizing_poc.py` | A/B comparison of `--om-wavelength-sizing` on Tokyo Bay vs. PoC #19's gradient-only baseline. Records alpha / frac<20° / NP / NE / shallow-cell density (≤ 5 m of water) / minimum CFL-feasible dt at C=0.7 to quantify whether the wavelength-driven sizing actually refines the shoaling regions and how much the implied dt drops. |
