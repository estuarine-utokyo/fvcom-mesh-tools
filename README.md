...
| 40 | `40_phase_h_dry_run_poc.py` | Stage 1 of the planned per-element greedy optimizer ("Phase H"). On the pipeline-rung-1 output of PoC #19 (47,426 elements), 12,440 (26.2 %) fail the strict per-element gate `alpha >= 0.95` ∧ `min_angle >= 20°`. Dry-run on each fail element of (a) `smooth_node` to the 1-ring centroid and (b) `edge_swap` of each internal edge: only **2 elements (0.02 %)** are improved by either operator. **Reason: pipeline rung 1 leaves the mesh at the simultaneous fixed point of Laplacian smoothing (Phase G) and Lawson edge swap (Phase D)** — moving a vertex to its neighbour-centroid is by construction a no-op, and every internal edge swap would flip a triangle (Delaunay-optimal). Implication: Phase H cannot make progress with smooth + swap alone; it must include topology-changing operators (`edge_split`, `vertex_remove` + 1-ring CDT, `edge_collapse`) plus a boundary-tangent smoother for the 5,717 fail elements (46 %) that touch a boundary. PoC drives the operator-inventory decision for the Phase H implementation. |
