...
| Under-resolved channels (medial-axis `w/h < min_w_h`) | `under_resolved_channels_flag` | **E** — `repair_under_resolved_channels` (centroid widen) | `--under-resolved-mode {widen,delete,none}` (default `none`); `--under-resolved-min-channel-elements N` filters out small isolated clusters (default `1` = no filter) | — | **rung 2** |
