"""Regression expectations for hires_walls_tools_review.md; failures are intentional.
...
def test_f6_smoke_rejects_zero_exit_without_completion(tmp_path):
    """F6: run only the real shell validation loop with a fake zero-exit solver.

    No scheduler, hostname guard, MPI runtime or real FVCOM is invoked.
    A placeholder output file exercises the script's existence-only check.
    """
    for case in ("base", "refined"):
        output = tmp_path / case / "output"
        output.mkdir(parents=True)
        (output / "partial.nc").write_bytes(b"not a completed NetCDF run")
    source = (ROOT / "jobs/octopus/423_m2_smoke.sh").read_text()
    loop = source[source.index("fail=0\n"):]
    shell = """set -euo pipefail
mpiexec() { echo 'STOP: integration ended early'; return 0; }
bc() { cat >/dev/null; echo 0; }
FVCOM=unused
RANKS=1
""" + loop
    result = subprocess.run(
        ["bash", "-c", shell], env={**os.environ, "SMOKE": str(tmp_path)},
...
def test_f11_resolve_adds_new_land_even_without_a_base_coast_stretch():
    """F11: an offshore hole has no free coastline and therefore no _shl."""
    ...
    rings, _ = island_rings(land, water, 30.0)
    assert len(rings) == 1  # This is resolvable land, far from the frozen interface.
    env = {
        "HIRES": {"coastline": "resolve"}, "_shl": None, "_filtered": land,
        "hole": water, "reports": {}, "island_rings": island_rings,
        "h_achieved": lambda q: np.full(len(q), 30.0),
    }
    exec(compile(ast.Module(body=[block], type_ignores=[]), "<driver block>", "exec"), env)
    assert env["reports"].get("islands_added", {}).get("n_islands_added") == 1
