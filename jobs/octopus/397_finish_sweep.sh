...
pids=()
run_variant obc0     "SR_H_TARGET=achieved SR_OBC_H0=814" & pids+=($!)
run_variant seed7    "SR_H_TARGET=achieved SR_FIN_SEED=7" & pids+=($!)
run_variant scale115 "SR_H_TARGET=achieved SR_DM_SCALE=1.15" & pids+=($!)
run_variant obc0seed "SR_H_TARGET=achieved SR_OBC_H0=814 SR_FIN_SEED=7" & pids+=($!)
status=0
for p in "${pids[@]}"; do wait "$p" || status=1; done
echo "=== variants done (status=$status) $(date -Is) ==="

for v in obc0 seed7 scale115 obc0seed; do
    mkdir -p "$COLLECT/$v"
