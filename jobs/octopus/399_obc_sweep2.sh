...
#PBS -N fmesh_399
#PBS -j o
#PBS -o logs/399_obc_sweep2.pbs.log
#PBS -r n
#============================================================================
...
# k=1.0 and h0=500 pass QA but lose time step (13.6 / 11.8 s vs 16.3);
# k=1.25 keeps dt 16.38 but fails at the NW junction. Try to keep the
# band as it is and relieve the junction itself.
run_variant skip2    "SR_H_TARGET=achieved SR_OBC_SKIP=2" & pids+=($!)
run_variant k115     "SR_H_TARGET=achieved SR_OBC_K=1.15" & pids+=($!)
run_variant k135     "SR_H_TARGET=achieved SR_OBC_K=1.35" & pids+=($!)
run_variant skip2k115 "SR_H_TARGET=achieved SR_OBC_SKIP=2 SR_OBC_K=1.15" & pids+=($!)
status=0
for p in "${pids[@]}"; do wait "$p" || status=1; done
echo "=== variants done (status=$status) $(date -Is) ==="

for v in skip2 k115 k135 skip2k115; do
    mkdir -p "$COLLECT/$v"
