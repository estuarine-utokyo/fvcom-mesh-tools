...
#PBS -N fmesh_401
#PBS -j o
#PBS -o logs/401_verify_best.pbs.log
#PBS -r n
#============================================================================
...
# Verify the chosen configuration reproduces on the current tree, and
# that the legacy interpretation still reproduces the certified mesh.
run_variant best   "SR_H_TARGET=achieved SR_OBC_SKIP=2" & pids+=($!)
run_variant legacy "SR_H_TARGET=field" & pids+=($!)
status=0
for p in "${pids[@]}"; do wait "$p" || status=1; done
echo "=== variants done (status=$status) $(date -Is) ==="

for v in best legacy; do
    mkdir -p "$COLLECT/$v"
