...
#PBS -N fmesh_402
#PBS -j o
#PBS -o logs/402_one_wide_continuity.pbs.log
#PBS -r n
# Compare policy-based one-row permission with the unchanged forbid paths.
# Submit from the repository root:
#   qsub jobs/octopus/402_one_wide_continuity.sh
set -euo pipefail
...
pids=()
run_variant field    "SR_H_TARGET=field SR_ONE_WIDE=forbid" & pids+=($!)
run_variant achieved "SR_H_TARGET=achieved SR_OBC_SKIP=2 SR_ONE_WIDE=forbid" & pids+=($!)
run_variant ach1w    "SR_H_TARGET=achieved SR_OBC_SKIP=2 SR_ONE_WIDE=allow" & pids+=($!)
status=0
for p in "${pids[@]}"; do wait "$p" || status=1; done
echo "=== variants done (status=$status) $(date -Is) ==="

for v in field achieved ach1w; do
    mkdir -p "$COLLECT/$v"
