...
#PBS -N fmesh_405
#PBS -j o
#PBS -o logs/405_coast_fit.pbs.log
#PBS -r n
# Validate the coastline-fitting pass: the same certified chain with the
# fit off and on, so the shoreline gain and its cost are measured on one
# machine, one input set, one run.
# Submit from the repository root:
#   qsub jobs/octopus/405_coast_fit.sh
set -euo pipefail
