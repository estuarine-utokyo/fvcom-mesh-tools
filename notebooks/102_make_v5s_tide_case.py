"""PoC #102 (stage 1) — M2 tide case for the v5 pipeline mesh.
...
MESH = REPO / "outputs" / "pipeline_v5s" / "tokyo_bay_v5s_final.14"
V2_INPUTS = REPO / "outputs" / "pipeline_v5s" / "fvcom_inputs"
TIDE_NML = REPO / "outputs" / "fvcom_tide_v4" / "tokyo_bay_v4_tide_run.nml"
CASE_DIR = REPO / "outputs" / "fvcom_tide_v5s"
CASENAME = "tokyo_bay_v5s_tide"
...
def main() -> int:
    ...
    for f in V2_INPUTS.glob("tokyo_bay_v5s_*.dat"):
    ...
    dep_case = CASE_DIR / "input" / "tokyo_bay_v5s_dep.dat"
    write_dep(clipped, dep_case)
    print(f"[102] test-only dep floor 5 m -> {dep_case}", flush=True)
    ...
    print(f"[102] wrote {nc_path}  (nobc={nobc}, nt={t.size})", flush=True)

    # --- namelist ----------------------------------------------------------
    txt = TIDE_NML.read_text()
    txt = txt.replace("tokyo_bay_v4_tide", CASENAME)
    txt = txt.replace("tokyo_bay_v4", "tokyo_bay_v5s")
    nml = CASE_DIR / f"{CASENAME}_run.nml"
    nml.write_text(txt)
    print(f"[102] wrote {nml}", flush=True)
    return 0
