"""PoC #16: river-inflow segments on the Tokyo Bay mesh.

The reference fort.14 already includes ibtype=21 segments for river
mouths around the bay (Edogawa, Arakawa, Sumidagawa, Tamagawa,
Tsurumigawa). PoC #14/15 produced a Tokyo Bay mesh with ibtype=20 land
segments only - this PoC plugs in
``data/rivers/tokyo_bay/tokyo_bay_rivers.csv`` via
``--river-inflow-points`` and confirms the resulting fort.14 has
matching ibtype=21 segments without disturbing mesh quality.

Outputs:
    outputs/16_tokyo_bay_with_rivers.14
    outputs/16_river_inflow_summary.txt
    outputs/16_river_inflow_mesh.png
"""
...
F14_OUT = OUT_DIR / "16_tokyo_bay_with_rivers.14"
SUMMARY_TXT = OUT_DIR / "16_river_inflow_summary.txt"
MESH_PNG = OUT_DIR / "16_river_inflow_mesh.png"
...
def plot_mesh(mesh, river_pts: np.ndarray, png: Path) -> None:
    ...
    # Open boundary in red, ibtype=20 land in gray, ibtype=21 rivers in blue.
    plotted_open = False
    ...
    for ib, seg in mesh.land_boundaries:
        if ib == 21:
            ax.plot(mesh.nodes[seg, 0], mesh.nodes[seg, 1], "-",
            ...
            plotted_land = True
    ax.scatter(river_pts[:, 0], river_pts[:, 1], marker="x",
...
def main() -> None:
    if not DEM.exists() or not COASTLINE.exists() or not RIVERS.exists():
    ...
    print("[16] running fmesh-buildmesh with rivers ...")
    t0 = time.perf_counter()
    rc = buildmesh_main(args)
    wall = time.perf_counter() - t0
    if rc != 0:
        raise SystemExit(f"buildmesh exited {rc}")
    print(f"[16] wall: {wall:.2f} s")
    ...
    print(f"[16] wrote {SUMMARY_TXT}")

    plot_mesh(mesh, river_pts, MESH_PNG)
    print(f"[16] wrote {MESH_PNG}")
