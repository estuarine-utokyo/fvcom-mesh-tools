...
DS = OM2D / "datasets"
DEG = 1.0 / 111e3
dem_p = DS/"SanJuan_1-9arcsec/san_juan_19_prvd02_2015.nc"
dp = DEM(str(dem_p)); bbox = dp.bbox
print(f"[sj] bbox={bbox}", flush=True)
reg = Region(tuple(bbox), 4326)
sh = Shoreline(str(DS/"SanJuan_1-9arcsec/sj_0contour_closed.shp"), reg.bbox,
               10.0*DEG)
...
print(f"[sj] mean acceptance = {np.nanmean(acc)*100:.2f}%", flush=True)
