"""PoC #51b: Pass E with the C5 + cascade-avoidance fixes.

Context — PoC #51 ran Pass E with only a C1/C2 gate and a candidate
list that included C4-fail edges. Results:

  * C4 102 -> 96  (just -6 over 66 accepts).
  * max_valence 8 -> 9, n_overconnected 0 -> 1  (C5 regression!).

Diagnosis: ``edge_split`` increases the valence of the two
"opposite" vertices by 1 each, but Pass E had no C5 check, so a
split at a node already at valence 8 pushed it to 9. And selecting
C4-fail edges as the candidate to split cascaded the violation onto
two new edges, yielding net-zero reductions for ~60 of the 66
accepts.

The Pass E operator now:

  1. Prefilters candidates with a C5 valence check — the opposite
     vertex of L (and the opposite vertex of the buddy triangle T
     for interior splits) must each be strictly below
     ``pass_e_max_valence``.
  2. Excludes any candidate edge that is itself in the current C4
     fail set. Splitting a non-fail candidate of L halves L's area
     and fixes every fail edge incident on L in one go.

This PoC re-runs the same Phase H configuration as PoC #51
(alpha=0.95, min_angle=30°, max_angle=130°, Pass A/B/C/D off,
Pass E on) on outputs/50_phase_h_fvcom_full_gate_optimized.14, but
with the corrected operator.

Outputs:
   outputs/51b_pass_e_fixed_optimized.14
   outputs/51b_pass_e_fixed_summary.txt
   outputs/51b_pass_e_fixed_summary.json
"""
...
OUTPUT = OUT_DIR / "51b_pass_e_fixed_optimized.14"
SUMMARY_TXT = OUT_DIR / "51b_pass_e_fixed_summary.txt"
SUMMARY_JSON = OUT_DIR / "51b_pass_e_fixed_summary.json"

POC51_SUMMARY = OUT_DIR / "51_pass_e_summary.json"

ALPHA_TARGET = 0.95
MIN_ANGLE_TARGET = 30.0
MAX_ANGLE_TARGET = 130.0
AREA_RATIO_TARGET = 0.5
MAX_VALENCE = DEFAULT_MAX_VALENCE  # 8 (FVCOM C5)
MAX_OUTER_ROUNDS = 10
MAX_SNAP_M = 500.0
...
def main() -> int:
    ...
    print(
        f"[51b] FVCOM residuals before Pass E: "
    ...
    print(
        f"[51b] building coastline projector from {COASTLINE.name} "
    ...
    print(f"[51b] projector built in {proj_build_wall:.1f} s", flush=True)

    print(
        f"[51b] running Pass E only (alpha={ALPHA_TARGET}, "
    ...
    print(
        f"[51b] Phase H (Pass E only) wall: {wall:.1f} s  "
    ...
    print(
        f"[51b] FVCOM residuals after Pass E: "
    ...
    poc51_payload = _load_json(POC51_SUMMARY)
    ...
    payload = {
        "config": {
            ...
            "lookahead_enabled": False,
            "patch_recdt_enabled": False,
            "operator_changes_vs_poc51": [
                "cascade_avoidance: skip C4-fail candidate edges",
                "C5 prefilter: reject if opposite vertex valence >= cap",
            ],
        },
        "input": {
    ...
    lines = [
        "PoC #51b — Pass E (C5 + cascade fix) on I1+I2 output",
        f"input: {INPUT.name}",
