"""PoC #52: full Phase H (Pass A + B + C + D + E) on the I1+I2
   output with the complete FVCOM gate stack.

Where we are after PoC #51d:

  v3 baseline           : C1 10  / C2 1 / C4 99  / C5 0  (PoC #48)
  I1+I2 (A+B only)      : C1 11  / C2 1 / C4 102 / C5 0  (PoC #50)
  + Pass E (swap+split) : C1 11  / C2 1 / C4 80  / C5 0  (PoC #51d)

Pass E alone delivers ``-22`` on C4 but leaves C1/C2 unchanged
because Pass E doesn't touch the angle gates directly. Likewise
Pass A+B alone gave up at C1=11 / C2=1, hitting the 1-ring local-
edit ceiling that PoC #44b previously measured at ~4.6 % under the
older v4.1 strict gate.

PoC #52 runs *every* pass simultaneously, letting them feed each
other across outer rounds:

  Pass A — batch Gauss-Seidel smooth (runs to exhaustion)
  Pass B — per-element greedy topology operators
  Pass C — 2-step lookahead on residual fails (target_exits_fail)
  Pass D — cluster patch re-CDT (size in [3, 100], strict gate)
  Pass E — gradation refinement (swap-first, then split fallback)

Hypothesis: Pass E's midpoints may unlock A/B/C/D accepts on the
next outer round (smoothing can reshape a sliver enough that
edge_swap or vertex_remove now passes the strict gate). Conversely
Pass A's smoothing may reshape a quad enough that Pass E's swap
test flips from CD-fail to CD-pass.

Wall budget: I1+I2 (A+B alone) was 1,937 s; PoC #51d (E alone) was
210 s. Combined run on the same input plus Pass C/D overhead
should land well under 1 h. Elapse cap 2 h.

Outputs:
   outputs/52_phase_h_full_optimized.14
   outputs/52_phase_h_full_summary.txt
   outputs/52_phase_h_full_summary.json
"""
...
OUTPUT = OUT_DIR / "52_phase_h_full_optimized.14"
SUMMARY_TXT = OUT_DIR / "52_phase_h_full_summary.txt"
SUMMARY_JSON = OUT_DIR / "52_phase_h_full_summary.json"
...
MAX_LOOKAHEAD_PER_ROUND = 10_000
MAX_PATCHES_PER_ROUND = 1_000
MAX_PASS_E_PER_ROUND = 10_000
...
def main() -> int:
    ...
    print(
        f"[52] input: NP={mesh_in.n_nodes:,}  NE={mesh_in.n_elements:,}",
        flush=True,
    )
    ...
    print(f"[52] projector built in {proj_build_wall:.1f} s", flush=True)

    print(
        f"[52] running full Phase H (A+B+C+D+E)\n"
    ...
    lines = [
        "PoC #52 — full Phase H (A+B+C+D+E) on I1+I2 output",
        f"input: {INPUT.name}",
    ...
    lines.append(
        f"  Pass C lookahead pairs        : "
        f"{sum(info.get('lookahead_pairs_applied', {}).values()):,}"
    )
    lines.append(
        f"  Pass D patch accepts          : "
        f"{sum(info.get('patch_recdt_accepts', {}).values()):,}"
    )
