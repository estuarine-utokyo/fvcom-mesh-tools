"""PoC #53: diagnose the residual FVCOM violations on PoC #52b's
   output.

PoC #52b (Phase H A+B+E) left:
    C1 (min_ang<30°)        : 9 fails
    C2 (max_ang>130°)       : 1 fail
    C4 (area_change>0.5)    : 80 fails
    C5 (valence>8)          : 0 ✓
    n_flipped               : 0 ✓

Why these 90 violations are stuck — that is, why every operator we
have tried (Pass A smooth, Pass B 1-step topology, Pass E swap +
indirect-regression-filtered split) refuses or undoes the fix — is
the question that decides the next architectural move. This PoC is
read-only and categorises each residual fail by:

  - cluster structure (face-face connected components for C1/C2;
    triangles touching a C4-fail edge for C4)
  - boundary proximity (does the cluster touch any open or land
    boundary segment?)
  - geographic extent (lon/lat bbox and centroid)
  - h_local (mean edge length on the fail elements)
  - for C4: the area-ratio extremity of each fail edge

Outputs:
    outputs/53_diagnostic_summary.txt
    outputs/53_diagnostic_summary.json
"""
...
INPUT = REPO / "outputs" / "52b_phase_h_abe_optimized.14"
OUT_DIR = REPO / "outputs"
SUMMARY_TXT = OUT_DIR / "53_diagnostic_summary.txt"
SUMMARY_JSON = OUT_DIR / "53_diagnostic_summary.json"
...
def main() -> int:
    ...
    NE = elements.shape[0]
    print(
        f"[53] input: NP={mesh.n_nodes:,}  NE={NE:,}",
        flush=True,
    )
    ...
    # ASCII summary
    lines = [
        "PoC #53 — residual FVCOM violation diagnostic",
        f"input: {INPUT.name}",
