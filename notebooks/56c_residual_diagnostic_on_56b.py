"""PoC #56c: per-fail diagnostic + C4 cluster analysis on PoC #56b output.

PoC #56b dropped C1 from 8 to 2 (spit triangles removed) but left
C4=68 untouched. Before designing Pass F we need to know, on the new
input (``56b_phase_h_min5_optimized.14``):

  * the per-element structure of the 2 remaining C1 fails (same
    treatment as PoC #56a: operator trials under the count-gate)
  * how the 68 C4 fails distribute spatially — are they isolated, or
    do they form clusters that a single cluster-patch reCDT could
    sweep at once?
  * for each cluster, the topology (boundary-touching extent, sizes)
    that determines whether a Pass-F-style boundary-aware operator is
    even applicable

This is read-only and runs on the login node in < 30 s.

Outputs:
    outputs/56c_residual_diagnostic.txt
    outputs/56c_residual_diagnostic.json
"""
...
SUMMARY_TXT = OUT_DIR / "56c_residual_diagnostic.txt"
SUMMARY_JSON = OUT_DIR / "56c_residual_diagnostic.json"
...
def main() -> int:
    ...
    print(
        f"[56c] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
        flush=True,
    )
    ...
    print(
        f"[56c] global: C1={int(c1_mask.sum())} "
    ...
    print(
        f"[56c] inspecting {len(c1_eids)} C1 fail elements", flush=True,
    )
    c1_records = []
    for eid in c1_eids:
    ...
    # ====================================================================
    print(
        f"[56c] analyzing {int(c4_edge_mask.sum())} C4 fail edges",
        flush=True,
    )
    ...
    # Text report
    lines = [
        "PoC #56c — residual diagnostic on PoC #56b output",
        f"input: {INPUT.name}",
