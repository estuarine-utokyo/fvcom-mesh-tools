"""PoC #58f: stochastic local fixer with tuned insert validation.

PoC #58e wired the insert operator in with a strict validation
(BARYCENTRIC_MIN = 0.10 and "every perimeter edge of the original
element must pass C4 ≤ 0.5") and observed **0** insert accepts over
~14 000 attempts. PoC #58f relaxes the two tuneable knobs:

  * ``BARYCENTRIC_MIN``: 0.10 → 0.05.  Sub-triangles now allowed to
    be as small as 5 % of the original along any barycentric coord.
  * Perimeter-edge C4 gate: now applied **only to the edges that
    were C4-failing on the input**.  The other two edges are
    permitted to land at any ratio post-insert, since they were
    already C4-clean and we still validate their sub-triangle
    angles and signed areas — i.e. we don't introduce new fails
    silently, but we don't reject a useful insert just because a
    previously-clean edge moves slightly toward the C4 threshold.

Move operator and the outer loop are otherwise identical to PoC #58e.
Instrumentation: per-outer-pass count of insert proposals vs
accepts, so we can confirm the relaxed gate is actually accepting.
...
OUTPUT = OUT_DIR / "58f_stochastic_tuned.14"
SUMMARY_TXT = OUT_DIR / "58f_summary.txt"
SUMMARY_JSON = OUT_DIR / "58f_summary.json"
...
BARYCENTRIC_MIN = 0.05      # PoC #58f: relaxed from 0.10
...
def _insert_local_quality_ok(
    ...
    area (strict — any regression rejects). C4 uses a
    **count-comparison gate** over the 3 perimeter edges + the 3
    new spoke edges:

      n_c4_before = number of perimeter edges of ``eid`` that are
          currently failing C4 (against their external buddies)
      n_c4_after  = same perimeter count post-insert (each
          sub-triangle vs the unchanged external buddy) + the 3
          spoke fail counts (sub vs sub)
      gate: ``n_c4_after <= n_c4_before``

    This allows the insert to *trade* a previously-failing
    perimeter edge for at most one new spoke fail without rejecting,
    which is the whole point of inserting into a C4-imbalanced
    element. C5: new vertex starts at valence 3; each of v0 / v1 /
    v2 gains 1 valence.
    """
    ...
    # C4 count-comparison gate.  Count BEFORE only over perimeter
    # edges of ``eid`` (the spoke edges don't exist yet).  Count
    # AFTER over perimeter (sub vs external buddy) + spokes (sub vs
    # sub).
    sub_for_edge = {
        (v0, v1): 0, (v1, v0): 0,
        (v1, v2): 1, (v2, v1): 1,
    ...
    # Pre-insert area of eid (whole, before split).
    tri_eid = mesh.elements[int(eid)]
    pe0 = mesh.nodes[int(tri_eid[0])]
    pe1 = mesh.nodes[int(tri_eid[1])]
    pe2 = mesh.nodes[int(tri_eid[2])]
    a_eid_before = float(abs(0.5 * (
        (pe1[0] - pe0[0]) * (pe2[1] - pe0[1])
        - (pe1[1] - pe0[1]) * (pe2[0] - pe0[0])
    )))
    ...
    for (a, b), buddy_id in proposal["external_buddies"].items():
        ...
        # Before: ratio between the whole eid and the buddy.
        larger_b = max(a_eid_before, a_buddy)
        smaller_b = min(a_eid_before, a_buddy)
        ratio_b = (larger_b - smaller_b) / max(larger_b, 1e-30)
        if ratio_b > AREA_RATIO_TARGET:
            n_c4_before += 1

        # After: ratio between the sub-triangle on this edge and the
        # unchanged external buddy.
        a_sub = sub_areas[sub_for_edge[(a, b)]]
        larger_a = max(a_sub, a_buddy)
        smaller_a = min(a_sub, a_buddy)
        ratio_a = (larger_a - smaller_a) / max(larger_a, 1e-30)
        if ratio_a > AREA_RATIO_TARGET:
            n_c4_after += 1

    # Spoke edges (sub vs sub).  These exist only after the insert.
    spoke_pairs = [
    ...
    # Count-comparison gate.
    if n_c4_after > n_c4_before:
        return False
...
def main() -> int:
    ...
    print(
        f"[58f] input : NP={before['NP']:,} NE={before['NE']:,} "
    ...
    for outer in range(1, MAX_OUTER_PASSES + 1):
        ...
        if not fail_set:
            print(f"[58f] outer pass {outer}: all fails cleared",
                  flush=True)
            break
        ...
        print(
            f"[58f] outer pass {outer}: {len(fail_list)} fail elements"
        ...
        print(
            f"[58f] outer pass {outer}: fixed={outer_fixed} "
            f"stuck={outer_stuck} avg_tries={avg_tries:.1f} "
            f"inserts={outer_inserts}",
            flush=True,
        )
        if outer_fixed == 0:
            print(f"[58f] no progress in outer pass {outer}; "
    ...
    print(
        f"[58f] output: NP={after['NP']:,} NE={after['NE']:,} "
