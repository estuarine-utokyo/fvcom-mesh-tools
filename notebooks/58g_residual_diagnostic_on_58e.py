"""PoC #58g: per-fail diagnostic on PoC #58e residual.

PoC #58e dropped 50 → 4 violations (1 C1 + 3 C4) via stochastic
move-only local fixing (inserts were dispatched at 30 % but
rejected 100 % of the time on sub-triangle angle gates). This
diagnostic reads ``58e_stochastic_with_insert.14`` and asks, for
each of the 4 surviving fails:

  * the C1 fail's per-edge structure, valences, and best-case
    single-operator delta (count-gate trial) — has the stochastic
    fixer shifted the C1 fail elsewhere, or is it still elem 4593 /
    12914 from PoC #56a?
  * the 3 C4 fails' cluster structure — are they part of the same
    quad clusters PoC #56c / #58a identified, or new isolated
    pairs created by the stochastic moves?
  * for each cluster, whether the fail edges are now boundary-
    corner-bound (no tangent → not reachable by move), or
    interior (where a smarter proposal — gradient-aware, or
    edge-swap — could clear them)

This is read-only and runs on the login node in < 5 s.

Outputs:
    outputs/58g_residual_diagnostic.txt
    outputs/58g_residual_diagnostic.json
"""
...
SUMMARY_TXT = OUT_DIR / "58g_residual_diagnostic.txt"
SUMMARY_JSON = OUT_DIR / "58g_residual_diagnostic.json"
...
def main() -> int:
    ...
    print(
        f"[58g] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
        flush=True,
    )
    ...
    print(
        f"[58g] global: C1={int(c1_mask.sum())} "
    ...
    print(
        f"[58g] inspecting {len(c1_eids)} C1 fail elements", flush=True,
    )
    c1_records = []
    for eid in c1_eids:
    ...
    # ====================================================================
    print(
        f"[58g] analyzing {int(c4_edge_mask.sum())} C4 fail edges",
        flush=True,
    )
    ...
    # Text report
    lines = [
        "PoC #58g — residual diagnostic on PoC #58e output",
        f"input: {INPUT.name}",
