"""PoC #58h: seed sweep for stochastic local fixer robustness.

Runs PoC #58e (move + insert dispatch, though insert never accepts
in practice) with seeds ``{42, 43, 44, 45, 46}`` against the same
PoC #57 Stage 1 input.  Reports the per-seed final
(C1, C2, C4, C5) residual + wall time so we can judge whether the
50 → 4 result on seed 42 is reproducible across seeds or
seed-specific.

Original PoC #58e module-level docstring follows:
...
SUMMARY_TXT = OUT_DIR / "58h_seed_sweep_summary.txt"
SUMMARY_JSON = OUT_DIR / "58h_seed_sweep_summary.json"
...
SEED_LIST: tuple[int, ...] = (42, 43, 44, 45, 46)


def _run_one(seed: int, input_mesh: Fort14Mesh, projector) -> dict:
    """Run the stochastic fixer for a single seed. Returns the
    per-seed summary record."""
    # Fresh mesh copy so each seed runs against the same input.
    mesh = Fort14Mesh(
        title=input_mesh.title,
        nodes=input_mesh.nodes.copy(),
        depths=input_mesh.depths.copy(),
        elements=input_mesh.elements.copy(),
        open_boundaries=[
            np.asarray(s).copy() for s in input_mesh.open_boundaries
        ],
        land_boundaries=[
            (int(ib), np.asarray(s).copy())
            for ib, s in input_mesh.land_boundaries
        ],
    )
...
def main() -> int:
    ...
    before = _global_metrics(mesh_in)
    print(
        f"[58h] input : NP={before['NP']:,} NE={before['NE']:,} "
    ...
    for seed in SEED_LIST:
        print(f"[58h] running seed = {seed} ...", flush=True)
        rec = _run_one(seed, mesh_in, projector)
        print(
            f"[58h]   seed={seed} → C1={rec['after']['C1']} "
            f"C2={rec['after']['C2']} C4={rec['after']['C4']} "
            f"C5={rec['after']['C5']} "
            f"total={rec['after']['C1'] + rec['after']['C2'] + rec['after']['C4'] + rec['after']['C5']} "
            f"wall={rec['wall_seconds']:.1f}s "
            f"fixed={rec['n_fixed']} stuck={rec['n_stuck']} "
            f"inserts={rec['n_inserts']}",
            flush=True,
        )
        records.append(rec)
    wall_sweep = time.perf_counter() - t_sweep

    # Aggregate stats across seeds.
    totals = [
        r["after"]["C1"] + r["after"]["C2"]
        + r["after"]["C4"] + r["after"]["C5"]
        for r in records
    ]
    c1s = [r["after"]["C1"] for r in records]
    c4s = [r["after"]["C4"] for r in records]
    ...
    summary = {
        ...
        "records": records,
        "stats": {
            "total_min": int(min(totals)),
            "total_max": int(max(totals)),
            "total_mean": float(np.mean(totals)),
            "total_median": float(np.median(totals)),
            "total_std": float(np.std(totals)),
            "c1_distribution": [int(x) for x in c1s],
            "c4_distribution": [int(x) for x in c4s],
            "wall_mean": float(np.mean(walls)),
            "wall_total": float(wall_sweep),
        },
    }
    ...
    lines = [
        f"PoC #58h — seed sweep ({len(SEED_LIST)} seeds × "
        f"PoC #58e fixer)",
        f"input : {INPUT.name}",
        f"before: C1={before['C1']} C2={before['C2']} "
        f"C4={before['C4']} C5={before['C5']} "
        f"(total {before['C1'] + before['C2'] + before['C4'] + before['C5']})",
        "",
        f"  {'seed':>5} | {'C1':>3} | {'C2':>3} | {'C4':>3} | "
        f"{'C5':>3} | {'total':>5} | {'wall (s)':>9} | "
        f"{'fixed':>5} | {'stuck':>5} | {'inserts':>7}",
        "  " + "-" * 75,
    ]
    for r in records:
        a = r["after"]
        t = a["C1"] + a["C2"] + a["C4"] + a["C5"]
        lines.append(
            f"  {r['seed']:>5} | {a['C1']:>3} | {a['C2']:>3} | "
            f"{a['C4']:>3} | {a['C5']:>3} | {t:>5} | "
            f"{r['wall_seconds']:>9.2f} | "
            f"{r['n_fixed']:>5} | {r['n_stuck']:>5} | "
            f"{r['n_inserts']:>7}"
        )
    lines.append("")
    lines.append(
        f"  total residual : "
        f"min={summary['stats']['total_min']} "
        f"max={summary['stats']['total_max']} "
        f"mean={summary['stats']['total_mean']:.1f} "
        f"median={summary['stats']['total_median']:.1f} "
        f"std={summary['stats']['total_std']:.2f}"
    )
    lines.append(f"  sweep wall     : {wall_sweep:.1f} s")
