...
RAW_OUT = REPO / "outputs" / "59j_v3_raw_100m.14"
STATS_JSON = REPO / "outputs" / "59j_fidelity_stats.json"
...
def main() -> int:
    ...
    print(f"[59j] building v3 raw mesh: hmin={HMIN_M} m, DEM={DEM.name}", flush=True)
    rc = buildmesh_main([
        str(DEM), str(RAW_OUT),
        "--engine", "oceanmesh",
    ...
    print(f"[59j] build wall: {build_wall:.0f} s (exit {rc})", flush=True)
    if rc != 0:
        raise SystemExit(f"buildmesh failed with exit {rc}")

    v3 = read_fort14(RAW_OUT)
    print(f"[59j] v3 raw: NP={v3.n_nodes:,} NE={v3.n_elements:,}", flush=True)
    stats = {
        "v3_raw_100m": {
    ...
    for name, zb in ZOOMS.items():
        plot_mesh_overview(
            v3, FIG_DIR / f"59j_v3_{name}.png", crs="EPSG:4326",
            cell_m=None, coast=(139.4, 35.05, 140.4, 35.85),
            zoom=zb, dpi=300,
            title=f"v3 raw 100 m — {name}",
        )
        if V1_MESH.exists():
            from pyproj import Transformer
            ...
            plot_mesh_overview(
                v1, FIG_DIR / f"59j_v1_{name}.png", crs="EPSG:32654",
                cell_m=None, coast=(139.4, 35.05, 140.4, 35.85),
                zoom=(x0, y0, x1, y1), dpi=300,
                title=f"v1 (200 m lineage) — {name}",
            )
    print(f"[59j] wall total: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
