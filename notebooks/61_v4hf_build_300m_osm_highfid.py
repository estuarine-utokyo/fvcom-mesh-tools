...
RAW_OUT = REPO / "outputs" / "61_v4hf_raw_300m_osm.14"
STATS_JSON = REPO / "outputs" / "61_fidelity_stats.json"
...
def main() -> int:
    ...
    print(f"[61] build wall: {time.perf_counter() - t0:.0f} s (exit {rc})",
          flush=True)
    if rc != 0:
        raise SystemExit(rc)
    mesh = read_fort14(RAW_OUT)
    stats = {
        "v4hf_raw_300m_osm_highfid": {
