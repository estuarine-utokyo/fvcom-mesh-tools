...
RAW_OUT = REPO / "outputs" / "62_v4hf2_raw_300m_osm.14"
STATS_JSON = REPO / "outputs" / "62_fidelity_stats.json"
...
def main() -> int:
    ...
    rc = buildmesh_main([
        ...
        "--om-seed", "0",
        "--om-high-fidelity-lines", str(COASTLINE),
        "--bbox-tol-m", "450",
    ...
    print(f"[62] build wall: {time.perf_counter() - t0:.0f} s (exit {rc})",
          flush=True)
    if rc != 0:
        raise SystemExit(rc)
    mesh = read_fort14(RAW_OUT)
    stats = {
        "v4hf2_raw_300m_osm_rawlines": {
