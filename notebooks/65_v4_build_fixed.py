...
RAW_OUT = REPO / "outputs" / "65_v4_raw_opened_fixed.14"
STATS_JSON = REPO / "outputs" / "65_fidelity_stats.json"
...
def main() -> int:
    ...
    print(f"[65] build wall: {time.perf_counter() - t0:.0f} s (exit {rc})",
          flush=True)
    if rc != 0:
        raise SystemExit(rc)
    mesh = read_fort14(RAW_OUT)
    stats = {
        "v4_raw_opened300_h0100_mult36_clamped": {
