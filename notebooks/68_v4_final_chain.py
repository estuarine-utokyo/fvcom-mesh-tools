"""PoC #68 — v4 (300 m, OSM) conform chain: the two user-flagged
...
OUT = REPO / "outputs" / "68_v4_gate.14"
QA_JSON = REPO / "outputs" / "68_v4_gate_qa.json"
LOG_JSON = REPO / "outputs" / "68_cycle_log.json"
...
def _apply_obc(mesh, ring, islands, open_end, trim=1):
    ...
    if hi - lo < 2:
        raise SystemExit("[68] open arc trimmed away")
...
def main() -> int:
    ...
    print(f"[68] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}", flush=True)
    ...
    if n_ch:
        ...
        mesh = _rebuild_ll(mesh)
    print(f"[68] channel exclusion (w/h < {MIN_WH:g}): removed {n_ch} "
    ...
    print(f"[68] OSM snap: {sinfo}", flush=True)
    for nodes_run, label in ((west_n, "west"), (south_n, "south")):
        if nodes_run.size >= 2:
            ...
            print(f"[68] OBC {label} line snap: {li}", flush=True)
    ...
    for cycle in range(MAX_CYCLES):
        ...
        print(f"[68] cycle {cycle}: failed = {sorted(failed)} {counts}",
              flush=True)
        ...
        if fingerprints.count(fp) >= 4:
            print("[68] stagnation — stopping", flush=True)
            break
        if "manifold_boundary" in failed:
            ...
            mesh = _apply_obc(mesh, ring, islands, open_end)
            print(f"[68] cycle {cycle}: deleted {int(bad.sum())} pinch elems",
                  flush=True)
            ...
            if bad.any():
                ...
                ring, islands, open_end, _w, _s = _arc_bands(mesh)
                mesh = _apply_obc(mesh, ring, islands, open_end)
                print(f"[68] cycle {cycle}: deleted {int(bad.sum())} R4/fake",
                      flush=True)
            else:
                mesh = _apply_obc(mesh, ring, islands, open_end, trim=2)
                print(f"[68] cycle {cycle}: junction residual — deeper trim",
                      flush=True)
            ...
            print(f"[68] cycle {cycle}: perp fixer accepted="
            ...
            print(f"[68] cycle {cycle}: unhandled — stopping", flush=True)
            break
        report = run_qa(mesh, name=OUT.name, path=OUT)
    ...
    if not (failed & FATAL_OR_QUALITY):
        ...
        for k, p in written.items():
            print(f"[68] export {k}: {p}", flush=True)
    else:
        print(f"[68] EXPORT SKIPPED: {sorted(failed & FATAL_OR_QUALITY)}",
              flush=True)
    print(f"[68] wall: {time.perf_counter() - t0:.1f} s", flush=True)
