"""PoC #68b (resume) — v4 (300 m, OSM) conform chain: the two user-flagged
...
SRC = REPO / "outputs" / "68_v4_gate.14"  # resume from #68 output
OSM_SHP = REPO / "outputs" / "osm_shoreline" / "osm_true_land_tokyo_bay.shp"
OSM_UTM_DIR = REPO / "outputs" / "osm_shoreline_utm_nocrs"
OUT = REPO / "outputs" / "68b_v4_gate.14"
QA_JSON = REPO / "outputs" / "68b_v4_gate_qa.json"
LOG_JSON = REPO / "outputs" / "68b_cycle_log.json"
...
def _apply_obc(mesh, ring, islands, open_end, trim=1):
    ...
    if hi - lo < 2:
        raise SystemExit("[68b] open arc trimmed away")
...
def main() -> int:
    ...
    print(f"[68b] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}", flush=True)

    from fvcom_mesh_tools.mesh_clean import weld_close_nodes

    # #68 left duplicates/tiny elements the ladder has no handler for.
    mesh, winfo = weld_close_nodes(mesh, tol=2.0)
    print(f"[68b] weld: {winfo}", flush=True)
    sa = _signed_areas_m(mesh)
    tiny = np.abs(sa) < 100.0   # < 100 m2 at a 75-300 m mesh = collapsed
    if tiny.any():
        mesh = remove_elements(mesh, ~tiny)
        mesh, _ = keep_components(mesh)
        print(f"[68b] dropped {int(tiny.sum())} tiny elements", flush=True)
    ...
    print(f"[68b] OSM snap: {sinfo}", flush=True)
    for nodes_run, label in ((west_n, "west"), (south_n, "south")):
        if nodes_run.size >= 2:
            ...
            print(f"[68b] OBC {label} line snap: {li}", flush=True)
    ...
    for cycle in range(MAX_CYCLES):
        ...
        print(f"[68b] cycle {cycle}: failed = {sorted(failed)} {counts}",
              flush=True)
        ...
        if fingerprints.count(fp) >= 4:
            print("[68b] stagnation — stopping", flush=True)
            break
        if "manifold_boundary" in failed:
            ...
            mesh = _apply_obc(mesh, ring, islands, open_end)
            print(f"[68b] cycle {cycle}: deleted {int(bad.sum())} pinch elems",
                  flush=True)
            ...
            if bad.any():
                ...
                ring, islands, open_end, _w, _s = _arc_bands(mesh)
                mesh = _apply_obc(mesh, ring, islands, open_end)
                print(f"[68b] cycle {cycle}: deleted {int(bad.sum())} R4/fake",
                      flush=True)
            else:
                mesh = _apply_obc(mesh, ring, islands, open_end, trim=2)
                print(f"[68b] cycle {cycle}: junction residual — deeper trim",
                      flush=True)
            ...
            print(f"[68b] cycle {cycle}: perp fixer accepted="
            ...
            print(f"[68b] cycle {cycle}: unhandled — stopping", flush=True)
            break
        report = run_qa(mesh, name=OUT.name, path=OUT)
    ...
    if not (failed & FATAL_OR_QUALITY):
        ...
        for k, p in written.items():
            print(f"[68b] export {k}: {p}", flush=True)
    else:
        print(f"[68b] EXPORT SKIPPED: {sorted(failed & FATAL_OR_QUALITY)}",
              flush=True)
    print(f"[68b] wall: {time.perf_counter() - t0:.1f} s", flush=True)
