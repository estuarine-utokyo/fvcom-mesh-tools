"""PoC #70 — manual-editing session v2: quality-gated snapping.
...
CKPT_A = REPO / "outputs" / "70_stageA_prepped.14"
CKPT_B = REPO / "outputs" / "70_stageB_ground.14"
OUT = REPO / "outputs" / "70_v4_session.14"
QA_JSON = REPO / "outputs" / "70_v4_session_qa.json"
SITES_JSON = REPO / "outputs" / "70_residual_sites.json"
FIG_DIR = REPO / "outputs" / "figures" / "70_sites"
...
def main() -> int:
    ...
    print(f"[70] A0 input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
          flush=True)
    ...
    if flag.any():
        ...
        mesh, _ = keep_components(mesh)
        mesh = _rebuild_ll(mesh)
    print(f"[70] A1 channel exclusion: -{int(flag.sum())} elems", flush=True)
    for rnd in range(12):
        ...
        mesh, _ = keep_components(mesh)
        mesh = _rebuild_ll(mesh)
    print(f"[70] A2 R4/fake deletion done (rounds={rnd + 1})", flush=True)
    ...
    deferred_snap = sinfo.pop("deferred_nodes")
    print(f"[70] A3 snap: {sinfo}", flush=True)
    ...
    print(f"[70] B optimize x1 done ({time.perf_counter() - t1:.0f} s) -> "
          f"{CKPT_B.name}", flush=True)

    # ---- C. ONE targeted perp pass ----------------------------------------
    mesh, pinfo = align_open_boundary_local(mesh, seed=7000, max_outer=1)
    print(f"[70] C perp pass: accepted={pinfo['accepted_total']} "
    ...
    mesh.title = "PoC 70 v4 session mesh (quality-gated snap, OSM-conformal)"
    ...
    SITES_JSON.write_text(json.dumps(sites, indent=2, ensure_ascii=False),
                          encoding="utf-8")
    print(f"[70] residual sites: {len(sites)} "
    ...
    for s in sites[:MAX_SITE_FIGS]:
        ...
        try:
            ...
            print(f"[70] site fig {s['site']} failed: {e}", flush=True)
    print(f"[70] site figures -> {FIG_DIR}", flush=True)
    ...
    if not (failed & FATAL):
        ...
        for k, pth in written.items():
            print(f"[70] export {k}: {pth}", flush=True)
    print(f"[70] wall: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
