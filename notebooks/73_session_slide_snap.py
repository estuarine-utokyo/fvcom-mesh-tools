"""PoC #73 — session v5: bisecting chain snap + on-line slide relax.
...
CKPT_A = REPO / "outputs" / "73_stageA_prepped.14"
CKPT_B = REPO / "outputs" / "73_stageB_ground.14"
OUT = REPO / "outputs" / "73_v4_session.14"
QA_JSON = REPO / "outputs" / "73_v4_session_qa.json"
SITES_JSON = REPO / "outputs" / "73_residual_sites.json"
FIG_DIR = REPO / "outputs" / "figures" / "73_sites"
...
def main() -> int:
    ...
    print(f"[73] A0 input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
          flush=True)
    ...
    if flag.any():
        ...
        mesh, _ = keep_components(mesh)
        mesh = _rebuild_ll(mesh)
    print(f"[73] A1 channel exclusion: -{int(flag.sum())} elems", flush=True)
    for rnd in range(12):
        ...
        mesh, _ = keep_components(mesh)
        mesh = _rebuild_ll(mesh)
    print(f"[73] A2 R4/fake deletion done (rounds={rnd + 1})", flush=True)
    ...
    if _oinfo.get("budget_exhausted"):
        print("[73] B: 10-min budget hit — continuing to targeted "
    ...
    print(f"[73] B optimize x1 done ({time.perf_counter() - t1:.0f} s) -> "
          f"{CKPT_B.name}", flush=True)

    # ---- C. ONE targeted perp pass ----------------------------------------
    mesh, pinfo = align_open_boundary_local(mesh, seed=7300, max_outer=1)
    print(f"[73] C perp pass: accepted={pinfo['accepted_total']} "
    ...
    mesh.title = "PoC 73 v4 session mesh (bisect + online-slide snap)"
    ...
    SITES_JSON.write_text(json.dumps(sites, indent=2, ensure_ascii=False),
                          encoding="utf-8")
    print(f"[73] residual sites: {len(sites)} "
    ...
    for s in sites[:MAX_SITE_FIGS]:
        ...
        try:
            ...
            print(f"[73] site fig {s['site']} failed: {e}", flush=True)
    print(f"[73] site figures -> {FIG_DIR}", flush=True)
    ...
    if not (failed & FATAL):
        ...
        for k, pth in written.items():
            print(f"[73] export {k}: {pth}", flush=True)
    print(f"[73] wall: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
