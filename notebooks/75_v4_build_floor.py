...
RAW_OUT = REPO / "outputs" / "75_v4_floor_h0100.14"
STATS_JSON = REPO / "outputs" / "75_fidelity_stats.json"
...
def main() -> int:
    ...
    print(f"[75] build wall: {time.perf_counter() - t0:.0f} s (exit {rc})",
          flush=True)
    if rc != 0:
        raise SystemExit(rc)
    mesh = read_fort14(RAW_OUT)
    stats = {
        "v4_floor_h0100_mult36": {
