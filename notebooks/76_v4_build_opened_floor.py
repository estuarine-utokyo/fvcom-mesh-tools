...
``outputs/osm_shoreline/osm_true_land_opened300.shp`` (60b clip).
...
COASTLINE = REPO / "outputs" / "osm_shoreline" / "osm_true_land_opened300.shp"
RIVERS = REPO / "data" / "rivers" / "tokyo_bay" / "tokyo_bay_rivers.csv"

RAW_OUT = REPO / "outputs" / "76_v4_opened_floor.14"
STATS_JSON = REPO / "outputs" / "76_fidelity_stats.json"
...
def main() -> int:
    ...
    print(f"[76] build wall: {time.perf_counter() - t0:.0f} s (exit {rc})",
          flush=True)
    if rc != 0:
        raise SystemExit(rc)
    mesh = read_fort14(RAW_OUT)
    stats = {
        "v4_opened_floor_h0100_mult36": {
