...
RAW_OUT = REPO / "outputs" / "77_v4_hmax1500.14"
STATS_JSON = REPO / "outputs" / "77_fidelity_stats.json"
...
def main() -> int:
    ...
    print(f"[77] build wall: {time.perf_counter() - t0:.0f} s (exit {rc})",
          flush=True)
    if rc != 0:
        raise SystemExit(rc)
    mesh = read_fort14(RAW_OUT)
    stats = {
        "v4_opened_floor_hmax1500": {
