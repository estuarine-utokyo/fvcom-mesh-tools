"""PoC #79 — true-300m session with boundary strip extrusion.
...
SRC = REPO / "outputs" / "77_v4_hmax1500.14"
OSM_SHP = REPO / "outputs" / "osm_shoreline" / "osm_true_land_tokyo_bay.shp"
OSM_UTM_DIR = REPO / "outputs" / "osm_shoreline_utm_nocrs"
CKPT_A = REPO / "outputs" / "79_stageA_prepped.14"
CKPT_B = REPO / "outputs" / "79_stageB_ground.14"
OUT = REPO / "outputs" / "79_v4_session.14"
QA_JSON = REPO / "outputs" / "79_v4_session_qa.json"
SITES_JSON = REPO / "outputs" / "79_residual_sites.json"
FIG_DIR = REPO / "outputs" / "figures" / "79_sites"
...
def main() -> int:
    ...
    print(f"[79] A0 input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
          flush=True)
    ...
    if flag.any():
        ...
        mesh, _ = keep_components(mesh)
        mesh = _rebuild_ll(mesh)
    print(f"[79] A1 channel exclusion: -{int(flag.sum())} elems", flush=True)
    for rnd in range(12):
        ...
        mesh, _ = keep_components(mesh)
        mesh = _rebuild_ll(mesh)
    print(f"[79] A2 R4/fake deletion done (rounds={rnd + 1})", flush=True)
    ...
    # A3a: fill the cleanup-retreat gap with a strip of new triangles
    # whose outer nodes sit exactly ON the coastline.
    mesh, xinfo = extrude_boundary_strip(
        mesh, lines, exclude_nodes=list(arc_all),
    )
    print(f"[79] A3a extrude: {xinfo}", flush=True)
    if xinfo["n_new_elements"]:
        ring, islands, open_end, west_n, south_n = _arc_bands(mesh)
    ...
    print(f"[79] A3 snap: {sinfo}", flush=True)
    ...
    if _oinfo.get("budget_exhausted"):
        print("[79] B: 10-min budget hit — continuing to targeted "
    ...
    print(f"[79] B optimize x1 done ({time.perf_counter() - t1:.0f} s) -> "
          f"{CKPT_B.name}", flush=True)

    # ---- C. ONE targeted perp pass ----------------------------------------
    mesh, pinfo = align_open_boundary_local(mesh, seed=7900, max_outer=1)
    print(f"[79] C perp pass: accepted={pinfo['accepted_total']} "
    ...
    for label, mask_fn in (
        ...
        if bad.any():
            mesh = remove_elements(mesh, ~bad)
            mesh, _ = keep_components(mesh)
            print(f"[79] E1 {label}: -{int(bad.sum())} elems", flush=True)
    ring, islands, open_end, _w, _s = _arc_bands(mesh)
    mesh = _apply_obc(mesh, ring, islands, open_end)
    arc_all = set(int(v) for v in ring[:open_end + 1])
    ...
    if deferred_snap_xy:
        ...
        for k in cand[order]:
            ...
            if res is not None:
                mesh = res[0]
                n_ins += 1
        print(f"[79] E2 inserts: {n_ins} new on-line nodes "
    ...
    print(f"[79] E3 snap pass 2: {sinfo2}", flush=True)

    # ---- D. final QA + residual-site report -------------------------------
    report = run_qa(mesh, name=OUT.name, path=OUT, max_offenders=100000)
    mesh.title = "PoC 79 v4 session mesh (extruded + session)"
    ...
    SITES_JSON.write_text(json.dumps(sites, indent=2, ensure_ascii=False),
                          encoding="utf-8")
    print(f"[79] residual sites: {len(sites)} "
    ...
    for s in sites[:MAX_SITE_FIGS]:
        ...
        try:
            ...
            print(f"[79] site fig {s['site']} failed: {e}", flush=True)
    print(f"[79] site figures -> {FIG_DIR}", flush=True)
    ...
    if not (failed & FATAL):
        ...
        for k, pth in written.items():
            print(f"[79] export {k}: {pth}", flush=True)
    print(f"[79] wall: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
