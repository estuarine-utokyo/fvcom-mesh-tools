"""PoC #82 (stage 1) — M2 tide case for the true-300m v4 mesh.
...
MESH = REPO / "outputs" / "81_v4_final.14"
V2_INPUTS = REPO / "outputs" / "fvcom_inputs_v4"
TIDE_NML = REPO / "outputs" / "fvcom_tide" / "tokyo_bay_v2_tide_run.nml"
CASE_DIR = REPO / "outputs" / "fvcom_tide_v4"
CASENAME = "tokyo_bay_v4_tide"
...
def main() -> int:
    ...
    for f in V2_INPUTS.glob("tokyo_bay_v4_*.dat"):
    ...
    print(f"[82] wrote {nc_path}  (nobc={nobc}, nt={t.size})", flush=True)

    # --- namelist ----------------------------------------------------------
    txt = TIDE_NML.read_text()
    txt = txt.replace("tokyo_bay_v2_tide", CASENAME)
    txt = txt.replace("tokyo_bay_v2", "tokyo_bay_v4")
    nml = CASE_DIR / f"{CASENAME}_run.nml"
    nml.write_text(txt)
    print(f"[82] wrote {nml}", flush=True)
    return 0
