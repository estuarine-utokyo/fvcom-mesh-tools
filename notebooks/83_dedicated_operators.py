"""PoC #83 — dedicated operators for the residual defect classes.
...
SRC = REPO / "outputs" / "81_v4_final.14"
OSM_SHP = REPO / "outputs" / "osm_shoreline" / "osm_true_land_tokyo_bay.shp"
OUT = REPO / "outputs" / "83_v4_final.14"
QA_JSON = REPO / "outputs" / "83_v4_final_qa.json"
LOG_JSON = REPO / "outputs" / "83_edit_log.json"
...
def main() -> int:
    ...
    print(f"[83] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
          flush=True)

    report0 = run_qa(mesh, name=SRC.name, path=SRC, max_offenders=10000)
    c1_off, c4_off = [], []
    ...
    for o, triple in zip(c1_off, triples):
        ...
        if e is None:
            edit_log.append({"target": sorted(triple), "op": "gone"})
            continue
        tri = [int(x) for x in mesh.elements[e]]
        P = mesh.nodes[tri]
        mn, _mx, _tw = _tri_quality(P[None, :, :])
        if mn[0] >= 30.0:
            edit_log.append({"target": tri, "op": "healed"})
            continue
        ...
        applied = False
        if n_bedges >= 1:
            for k in order:  # shortest edge first
                a, b = pairs[int(k)]
                res = collapse_edge(mesh, a, b, lines=lines,
                                    protected=protected)
                if res is not None:
                    mesh, info = res
                    edit_log.append({"target": tri, "op": "collapse",
                                     **info})
                    applied = True
                    break
        else:
            for k in order[::-1]:  # longest interior edge first
                a, b = pairs[int(k)]
                res = split_edge_pair(mesh, a, b)
                if res is not None:
                    mesh, info = res
                    edit_log.append({"target": tri, "op": "split",
                                     **info})
                    applied = True
                    break
        if not applied:
            edit_log.append({"target": tri, "op": "unresolved",
                             "min_angle_deg": float(mn[0])})
        print(f"[83] C1 {tri} ({o['min_angle_deg']:.1f} deg): "
              f"{edit_log[-1]['op']}", flush=True)

    # ---- pass 2: C4 offenders (still-failing shared edges) -----------------
    for o in c4_off:
        ...
        if carriers.size != 2:
            edit_log.append({"target": [u, v], "op": "edge-gone"})
            continue
        tri = mesh.elements[carriers]
        _mn, _mx, twice = _tri_quality(mesh.nodes[tri])
        a1, a2 = 0.5 * np.abs(twice)
        hi = max(a1, a2)
        if hi <= 0 or (hi - min(a1, a2)) / hi <= 0.5:
            edit_log.append({"target": [u, v], "op": "healed"})
            continue
        res = split_edge_pair(mesh, u, v)
        if res is not None:
            mesh, info = res
            edit_log.append({"target": [u, v], "op": "split", **info})
        else:
            res = collapse_edge(mesh, u, v, lines=lines,
                                protected=protected)
            if res is not None:
                mesh, info = res
                edit_log.append({"target": [u, v], "op": "collapse",
                                 **info})
            else:
                edit_log.append({"target": [u, v], "op": "unresolved",
                                 "area_change": float(o["area_change"])})
        print(f"[83] C4 ({u},{v}) ({o['area_change']:.2f}): "
              f"{edit_log[-1]['op']}", flush=True)

    # ---- finalize -----------------------------------------------------------
    mesh, cinfo = compact_nodes(mesh)
    print(f"[83] compact: {cinfo}", flush=True)
    ...
    if not (failed & FATAL):
        ...
        for k, pth in written.items():
            print(f"[83] export {k}: {pth}", flush=True)
    print(f"[83] wall: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
