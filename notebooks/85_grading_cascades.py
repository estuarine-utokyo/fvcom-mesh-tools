"""PoC #85 — grading cascades for the size-cliff C4 residuals.
...
SRC = REPO / "outputs" / "84_v4_final.14"
OSM_SHP = REPO / "outputs" / "osm_shoreline" / "osm_true_land_tokyo_bay.shp"
OUT = REPO / "outputs" / "85_v4_final.14"
QA_JSON = REPO / "outputs" / "85_v4_final_qa.json"
LOG_JSON = REPO / "outputs" / "85_edit_log.json"
...
def _one_pass(mesh, lines, protected, edit_log, pass_no):
    ...
    print(f"[85] pass {pass_no}: C1={len(c1_off)} C4={len(c4_off)}",
          flush=True)
    ...
    for o, triple in zip(c1_off, triples):
        ...
        print(f"[85] p{pass_no} C1 {tri} ({o['min_angle_deg']:.1f}): "
    ...
    for o in c4_off:
        ...
        if hi <= 0 or (hi - min(a1, a2)) / hi <= 0.5:
            continue
        applied = "unresolved"
        res = grade_region(mesh, u, v, max_splits=8)
        if res is not None:
            mesh, info = res
            applied = "grade"
        ...
        print(f"[85] p{pass_no} C4 ({u},{v}) ({o['area_change']:.2f}): "
              f"{applied}", flush=True)
    return mesh
...
def _fix_perp_node(mesh, edit_log):
    ...
    for v in nodes_bad:
        ...
        print(f"[85] perp node {v}: {applied}", flush=True)
    return mesh
...
def main() -> int:
    ...
    print(f"[85] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
          flush=True)
    ...
    print(f"[85] compact: {cinfo}", flush=True)
    ...
    if not (failed & FATAL):
        ...
        for k, pth in written.items():
            print(f"[85] export {k}: {pth}", flush=True)
    print(f"[85] wall: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
