"""PoC #86 — equalize_pair: the drag-the-interior-node move.

Splitting trades C4 for new C1 (angle halving), so #85 cleared
nothing. equalize_pair moves the two apex nodes of a C4 pair
(interior only) — big apex toward the shared edge, small apex
away — via a deterministic joint grid search, the direct SMS
rebalancing edit. C1 elements get the same treatment on each of
their edges.
...
SRC = REPO / "outputs" / "85_v4_final.14"
OSM_SHP = REPO / "outputs" / "osm_shoreline" / "osm_true_land_tokyo_bay.shp"
OUT = REPO / "outputs" / "86_v4_final.14"
QA_JSON = REPO / "outputs" / "86_v4_final_qa.json"
LOG_JSON = REPO / "outputs" / "86_edit_log.json"
...
def _one_pass(mesh, lines, protected, edit_log, pass_no):
    ...
    print(f"[86] pass {pass_no}: C1={len(c1_off)} C4={len(c4_off)}",
          flush=True)
    ...
    for o, triple in zip(c1_off, triples):
        ...
        print(f"[86] p{pass_no} C1 {tri} ({o['min_angle_deg']:.1f}): "
    ...
    for o in c4_off:
        ...
        print(f"[86] p{pass_no} C4 ({u},{v}) ({o['area_change']:.2f}): "
              f"{applied}", flush=True)
    return mesh
...
def _fix_perp_node(mesh, edit_log):
    ...
    for v in nodes_bad:
        ...
        print(f"[86] perp node {v}: {applied}", flush=True)
    return mesh
...
def main() -> int:
    ...
    print(f"[86] input: NP={mesh.n_nodes:,} NE={mesh.n_elements:,}",
          flush=True)
    ...
    print(f"[86] compact: {cinfo}", flush=True)
    ...
    if not (failed & FATAL):
        ...
        for k, pth in written.items():
            print(f"[86] export {k}: {pth}", flush=True)
    print(f"[86] wall: {time.perf_counter() - t0:.0f} s", flush=True)
    return 0
