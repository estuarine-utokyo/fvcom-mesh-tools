"""The second review's findings (docs/hires_walls_tools_review_2.md) as
regression tests: written as failing probes by the reviewer (gpt-6-astra),
passing since the fixes; R1 adapted to the invalidation now running first."""
...
def test_r1_failed_prerequisite_invalidates_old_marker(tmp_path, script, start, stop, marker):
    """The stage's own invalidation (which runs first, before the environment
    set-up) followed by its prerequisite check: the check fails, and no marker
    from an earlier attempt is left."""
    old = tmp_path / marker
    ...
    env = {**os.environ, "OUTDIR": str(tmp_path / "unaccepted"),
           ...
           "CASE": "base", "FMESH_CASE": "base", "FMESH_REQUIRE_SMOKE": "1"}
